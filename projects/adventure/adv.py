...
def other_direction(direction):
 ...
 # create a function that will explore the graph and find path direction

 
s = Stack()
traversalGraph = {} # dictionary for graph traversal
...
while len(traversalGraph) < len(roomGraph):
    ...
    if player.currentRoom.id not in traversalGraph:    
        ...
        for exit in player.currentRoom.getExits():
            exiting[exit] = '?' 
        # assign exit to the current room in the graph
        traversalGraph[player.currentRoom.id] = exiting
    ...
    for direction, exit_room in exiting.items():
        if exit_room == '?':
            ...
            if exiting[direction] not in traversalGraph:
               ...
               #reassign the current room
                ...
                open_exits = True
                traversalGraph[exiting[direction]]
                
                if open_exits:
                    # If room exists travel to the previous room/opposite direction **
            ...
            open_exits = True  # Indicates that we found an exit.
            break

    if not open_exits: # We come here if we end up in the room we already been
        going_back = s.pop()
        
        # Stopping the loop in we go back to the original room
        if going_back == None:
            break
